...
#define INNIR_UPDATE_INTERVAL       SECONDS(3)
...
#define MLOOP_EVT_TIME_UPDATE       (1 << 4)
...
static void outdoor_update(void)
{
	...
	const char *weather_url = "https://api.seniverse.com/v3/weather/now.json?key=SxhoftKThl5jyNAPN&location=WQJ6YY8MHZP0&language=en&unit=c";
...
static void mloop_func(void *pararm)
{	
	uint32_t event;
...
void mloop_init(void)
{
	...
	time_sync_timer = xTimerCreate("time sync", pdMS_TO_TICKS(1), pdFALSE, (void *)MLOOP_EVT_TIME_SYNC, mloop_timer_cb);
	wifi_update_timer = xTimerCreate("wifi update", pdMS_TO_TICKS(WIFI_UPDATE_INTERVAL), pdTRUE, (void *)MLOOP_EVT_WIFI_UPDATE, mloop_timer_cb);
	inner_update_timer = xTimerCreate("inner update", pdMS_TO_TICKS(INNIR_UPDATE_INTERVAL), pdTRUE, (void *)MLOOP_EVT_INNER_UPDATE, mloop_timer_cb);
	outdoor_update_timer = xTimerCreate("outdoor update", pdMS_TO_TICKS(OUTDOOR_UPDATE_INTERVAL), pdTRUE, (void *)MLOOP_EVT_OUTDOOR_UPDATE, mloop_timer_cb);
	
	xTaskCreate(mloop_func, "mloop", 1024, NULL, 5, NULL);
