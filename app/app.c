...
static void time_update(void)
{
    static rtc_date_time_t last_date = {0};//记忆上一次状态

    rtc_date_time_t date;
    rtc_get_time(&date);//进行比较短时间内的秒数是否一样，比如防止23：59：59秒和0：0：0这种情况，由于时未更新，读取的时间时23：0：0
...
void mloop_init(void)//申请定时器和任务，
{
	...
	inner_update_timer = xTimerCreate("inner update", pdMS_TO_TICKS(INNIR_UPDATE_INTERVAL), pdTRUE, inner_update, work_timer_cb);
