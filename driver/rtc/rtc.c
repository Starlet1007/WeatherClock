...
static void _rtc_set_time_once(const rtc_date_time_t *date_time)//单次设置时间，不等待同步
{
	...
	date.RTC_Year = date_time->year - 2000;//F4硬件部分存储范围是0-99
...
static void _rtc_get_time_once(rtc_date_time_t *date_time)//单次获取时间，不等待同步
{
	RTC_DateTypeDef date;
	RTC_TimeTypeDef time;
	RTC_DateStructInit(&date);
	...
	date_time->year = 2000 + date.RTC_Year;//将硬件读取到的年份转换为2000+年份
...
void rtc_set_time(const rtc_date_time_t *date_time)//防止比如设置时间为2022-12-31 23:59:59，瞬间后获取到的时间还是2023-01-01 00:00:00
{
